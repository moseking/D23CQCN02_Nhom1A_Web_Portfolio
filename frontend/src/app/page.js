...
const works = [
  ...
  },
  {
    ...
    author: "Vy",
    avatar: "https://images.unsplash.com/photo-1509967419530-da38b4704bc6?auto=format&fit=crop&w=160&q=80",
...
const creators = [
  ...
  },
  {
    ...
    followers: "9.8K",
    avatar: "https://images.unsplash.com/photo-1509967419530-da38b4704bc6?auto=format&fit=crop&w=200&q=80",
  },
  {
    name: "Huy Nguyen",
    role: "Illustrator",
...
export default function Home() {
      ...
      try {
        ...
        const postsWithComments = await Promise.all(
          posts.map(async (post) => {
            const commentResponse = await fetch(`${API_URL}/api/posts/${post.id}/comments`, {
              signal: controller.signal,
            });
  ...
  const filteredWorks = useMemo(() => {
    ...
    return allWorks.filter((work) => {
      const matchesCategory =
        activeCategory === "All" ||
        work.tags.some((tag) => tag.toLowerCase().includes(activeCategory.toLowerCase()));
      const matchesSearch =
        !keyword ||
        [work.title, work.author, work.content || "", ...work.tags].some((item) =>
  ...
  const addCommentToPost = (postId, comment) => {
    setApiWorks((currentWorks) =>
      currentWorks.map((work) =>
        work.id === postId ? { ...work, comments: [comment, ...(work.comments || [])] } : work
      )
    );
  };
  ...
  return (
    <main className="min-h-screen bg-[#f7f8f3] text-[#252525]">
      ...
      <section className="mx-auto max-w-[1840px] px-6 py-20 sm:px-10 lg:px-12" id="explore">
      ...
      <section className="border-y border-[#e0e3da] bg-white/55 px-6 py-20 sm:px-10 lg:px-12" id="creators">
          ...
          <form className="comment-form" onSubmit={submitComment}>
            ...
            <div>
              <input
                ...
                placeholder={work.id ? "Write a comment..." : "Comments are enabled for saved posts"}
                required
                type="text"
                value={content}
              />
...
function CreatorCard({ creator, index }) {
  return (
    <article className="creator-card reveal" style={{ "--delay": `${index * 80}ms` }}>
